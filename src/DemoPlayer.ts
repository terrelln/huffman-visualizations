...
const DEMO_DESCRIPTIONS = [
  'Builds a Huffman tree from symbol frequencies using the <a href="https://en.wikipedia.org/wiki/Huffman_coding">double-queue algorithm</a>. Two sorted queues (leaves and merged nodes) are maintained; each step dequeues the two lowest-frequency items, merges them into a new parent node, and enqueues the result. The process repeats until a single root remains.',
  'Encodes a string using the Huffman tree. For each character, the tree is traversed from root to the corresponding leaf \u2014 each left edge produces a 0 bit and each right edge produces a 1. The concatenated bits form the codeword for that character.',
  ...
  'Enforces a maximum code length on the Huffman code to limit the size of the decoding table (shown in the last two demos), which grows exponentially with the maximum depth. Codes exceeding the depth limit are clamped, then a demote/promote pass redistributes code lengths to satisfy the <a href="https://en.wikipedia.org/wiki/Kraft%E2%80%93McMillan_inequality">Kraft inequality</a> while keeping total encoding cost low. The algorithm used is a simple heuristic that works well in practice but does not guarantee an optimal result. The <a href="https://en.wikipedia.org/wiki/Package-merge_algorithm">Package-Merge algorithm</a> produces an optimal length-limited code, but in practice the difference is negligible and the heuristic is faster.',
  'Encodes a string by looking up each character in the canonical symbol table produced by depth limiting. Instead of traversing a tree, each character maps directly to its codeword via a table lookup. This is how real-world implementations typically perform encoding, since it is much faster than tree traversal.',
  'Constructs a single-level decoding lookup table with 2<sup>D</sup> entries, where D is the max depth. Each symbol\u2019s codeword is padded to D bits, and all table slots sharing that prefix are filled with the symbol and its actual bit length.',
  'Decodes a bitstring using the lookup table. For each codeword, D bits are read from the stream and used as an index into the table. The entry gives the decoded symbol and how many bits to consume, then the stream advances.',
];
